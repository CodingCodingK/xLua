...
namespace XLua.CSObjectWrap
{
    ...
    public class PeopleEnumWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(PeopleEnum), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(PeopleEnum), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(PeopleEnum), L, null, 3, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Male", PeopleEnum.Male);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Female", PeopleEnum.Female);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(PeopleEnum), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            {
                translator.PushPeopleEnum(L, (PeopleEnum)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {

			    if (LuaAPI.xlua_is_eq_str(L, 1, "Male"))
                {
                    translator.PushPeopleEnum(L, PeopleEnum.Male);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Female"))
                {
                    translator.PushPeopleEnum(L, PeopleEnum.Female);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for PeopleEnum!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for PeopleEnum! Expect number or string, got + " + lua_type);
            }
